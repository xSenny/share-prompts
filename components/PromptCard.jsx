        ...
        <div className={"prompt_card"}>
            <div className={"flex justify-between items-start gap-5"}>
                <div className={"flex-1 flex justify-start items-center gap-3 cursor-pointer"}>
                    <Image onClick={handleProfileForwarding} src={post.creator.image} alt={"user_image"} width={40} height={40} className={"rounded-full object-contain"}/>
                    <div className={"flex flex-col"}>
                        <h3 onClick={handleProfileForwarding} className={"font-satoshi font-semibold text-gray-900"}>{post.creator.username}</h3>
                        <p onClick={handleProfileForwarding} className={"font-inter text-sm text-gray-500"}>{post.creator.email}</p>
                    </div>
                </div>
            ...
            <p className={"my-4 font-satoshi text-sm text-gray-700 cursor-pointer"} onClick={() => goToPromptPage()}>{post.prompt}</p>
